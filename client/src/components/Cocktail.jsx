...
export default function Cocktail({cocktail, selectedIngs, handleRemoveFromFavourites, user, setUser, page}) {
  ...
  if(handleRemoveFromFavourites) return (
    <div className="Cocktail">
      ...
      <button className='fave-button' onClick={()=>handleRemoveFromFavourites(user,cocktail.idDrink)}>Remove From Favourites</button>
     </div>
  )
  ...
  if(!user.username) return (
    <div className="Cocktail">
      ...
      <div className="cocktail-details">
         <h2>{cocktail.strDrink}</h2>
         <div>
         {ingredientsWithMeasures.map(ing=> <p key={ing} className={comparisonArr.includes(ing) ? "matched-ing" : 'ing'}>{ing}</p>)}
         </div>
  ...
  return (
    ...
    <div className="Cocktail">
      ...
      <button className='fave-button' onClick={()=> toggleFave(user,cocktail.idDrink)}>Remove From Favourites</button>
    </div>
     :
    <div className="Cocktail">
      <img className='cocktail-img' src={cocktail.strDrinkThumb}/>
      ...
      <button className='fave-button' onClick={()=> toggleFave(user,cocktail.idDrink)}>Add To Favourites</button>
     </div>
    }
    </>
  )
